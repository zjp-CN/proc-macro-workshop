...
// Resources:
//
//   - Relevant syntax tree types: https://docs.rs/syn/1.0/syn/struct.Attribute.html https://docs.rs/syn/1.0/syn/enum.Meta.html
...
fn main() {
    let command = Command::builder().executable("cargo".to_owned())
                                    .arg("build".to_owned())
                                    .arg("--release".to_owned())
                                    .env("LTO=true".to_owned())
                                    .build()
                                    .unwrap();

    assert_eq!(command.executable, "cargo");
    assert_eq!(command.env, vec!["LTO=true"]);
